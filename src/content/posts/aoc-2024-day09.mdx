...
I'm not going to copy the full function here because I'm not overly proud of it (although it's still available on https://github.com/martapanc/Advent-of-Code/blob/master/2024/src/2024/day09/day09.ts), but here's the steps:
...
- While `blockId` is greater than 0:
    ...
    - Locate the first empty block (`.`) that is large enough to accommodate `blockLength`

        - If no such block exists or the empty block is located after `blockIndex`, decrement `blockId` and continue.

        - Otherwise, retrieve and process the empty block.

    - If the `blockLength` is smaller than or equal to the empty block:

    - **Block Length < Empty Length:**

        - Replace the empty block with a block of `blockId` and size `blockLength`.
    ...
    - Decrement `blockId` and repeat the loop until `blockId` reaches 0.
