...
function calcHeightPerPin(key: Map<string, string>) {
    const array: number[] = [];
    for (let x = 0; x < 5; x++) {
        const height = [...key.entries()].filter(([cell, val]) => cell.x === x && val === '#').length - 1;
        array.push(height);
    }
    return array;
}
...
And this is it! As per tradition, we can obtain the 50th star for free as long as we have the previous 49 in store - so clearly no Part 2 puzzle in this article! :D
